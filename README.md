...
<br>
<br>
Front 작업을 할 때 처음 배운 Javascript로 만든 페이지에 원하는 기능을 구현함에 있어 어려웠습니다.
clickevent를 주어서 눌렀을 때 기능이 구현되게 하거나, 모달창을 만드는 방식을 구현하면서 어떻게 하면 가독성 있고 보기에 편한 페이지를 만들 수 있는지 오랫동안 고민했습니다.
<br>
<br>
REST API를 사용하면서 REST 방식으로 가져온 데이터를 fetch 방식으로 불러오고 수정하는 방법에서 어려움을 겪었습니다. 
fetch를 통해 가져온 데이터를 View에서 수정한 후 html로 보낼때, 각 과정마다 데이터가 제대로 불러와졌는지 확인하는 과정에서 시간을 많이 할애했습니다. 특히, 파일을 다운로드 받는 부분과 표로 정리되어있는 데이터 중 선택해서 그 데이터를 골라서 삭제하는 등의 다양한 기능을 구현할 때 더욱 고난을 겪었습니다.
<br>
<br>
허나 위 경험을 통해 웹페이지를 만들 때 어떻게 데이터베이스에서 원하는 데이터를 가져오는지에 대한 이해와 역량이 크게 향상되었습니다. 프로젝트를 성공적으로 완수하기 위해 고민하는 과정에서, 웹페이지와 서버간의 통신방법에서 동기와 비동기 통신의 구분과 그 이해력이 크게 향상되었습니다. 이를 깨닫는 과정을 거치며 저는 이번 프로젝트를 성공적으로 마무리 지을 수 있었으며, 앞으로의 프로젝트에서도 원하는 데이터를 명확히 가져와서 기능을 구현할 수 있게 되었습니다.
<br>
<br>
<h3>✨문제를 해결했던 부분</h3>

다운로드 기능을 구현해야 했습니다. 파일다운로드라는 버튼을 클릭했을 때, clickevent가 실행되면서 javascript를 통해 비동기 통신 방식으로 fetch를 사용하여 DownloadView로 넘어가는 과정에서 계속해서 에러가 나타나는 문제가 발생했습니다.
<br>
<br>
print()와 console.log를 이용하여 어디에서 문제가 발생하는가를 하나씩 확인한 결과, fetch를 통해 DownloadView로 통신할 때 urls.py로 이동하는 부분에서 오류가 있었습니다. 코드에서 상대경로를 제대로 찾지 못한것이 원인이었습니다.
<br>
<br>
...
<br>
<br>
이를 해결하기 위해, Model에서 imagefile과 downloadfile을 따로 구분지어 다시 Model을 만들었고 Migrate를 진행한 다음, 그 downloadfile을 DownloadView에서 가져오도록 코드를 수정했습니다. 그 후, 한글이름을 가진 파일의 인코딩을 위해 utf-8로 인코딩을 변경해주었습니다.
<br>
<br>
...
<br>
<br>
프로젝트를 시작할 때 6명의 팀원으로 출발했습니다. 그러나 프론트엔드 작업이 마무리되는 동안 한 명의 팀원이 개인 사정으로 인해 팀을 떠나게 되었고, 그 결과 우리 팀은 5명으로 줄어들었습니다. 한편, 다른 팀에서도 멤버들이 떠나면서 해당 팀이 유지될 수 없게 되었고, 그 결과 우리 팀에 한 명이 합류하게 되었습니다.
<br>
<br>
이로 인해 팀 내 분위기가 어수선해졌지만, 그래도 프로젝트를 잘 마무리해보자라고 단결되고 있었습니다. 하지만 팀장님이 부득이한 사정으로 참여하지 못하게 되었습니다. 저희 팀은 다른 팀에 비해 실력이 월등하게 좋은 편은 아니었습니다. 그래도 팀장님과 팀원간의 노력으로 서로간의 의견을 잘 융화시켜 원만하게 프로젝트를 수행하고 있었으나, 더는 그러지 못하게 되자 팀내 갈등이 생기게 되었습니다.
<br>
<br>
프로젝트를 성공적으로 완수하기 위해서는 이러한 갈등 상황을 중재하고 해소해야 했습니다. 이를 위해 저는 팀 내 의견 불일치로 인한 갈등 상황을 중재하고자 더욱 팀 회의를 자주 열자고 제안했습니다. 의견 충돌을 회의와 원만한 대화를 통해 해결하고, 팀 인원의 감소를 핑계로 삼지 않고 오히려 책임을 더욱 키우고자 했습니다.
<br><br>
각자의 역할과 책임을 분명하게 정의하고, 그에 따라 효율적으로 협업하기로 했습니다. 프로젝트 진행 중에 발생한 문제들도 우리가 서로의 의견을 존중하고 소통을 통해 해결해 나갔기 때문에 큰 장애물도 헤쳐나갈 수 있었습니다. 회의를 열고 문제점을 함께 나누며 찾아낸 해결책들은 우리를 더 가깝게 이끌어 주었습니다. 리더십의 중요성도 역시 부각되었습니다. 저는 팀원들과의 대화를 통해 모두가 자신의 역할을 이해하고 최선을 다할 수 있도록 도왔습니다. 
<br><br>
